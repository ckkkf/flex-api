# Gemini格式 模块接口文档

- 来源：`new-api-main/docs/openapi/relay.json` + `new-api-main/router/relay-router.go` / `video-router.go`
- OpenAPI 覆盖接口数量：2

## 模块说明

- Relay 接口通常直接透传 OpenAI / Claude / Gemini / 视频平台兼容协议。
- 绝大多数接口需要令牌认证；少数下载类接口支持用户会话或令牌二选一。

## 接口明细

### POST /v1/engines/{model}/embeddings

- 摘要：Gemini 嵌入(Embeddings)
- 请求方法：`POST`
- 路径：`/v1/engines/{model}/embeddings`

#### 路径 / Query 参数

| 字段 | 类型 | 含义 | 取值范围 |
|---|---|---|---|
| model | path / string / 必填 | 模型/引擎 ID | - |

#### 请求体

- Content-Type：`application/json`
- 请求体结构：`EmbeddingRequest`

| 字段 | 类型 | 含义 | 取值范围 |
|---|---|---|---|
| model | string | 请求体字段 | - |
| input | object | 请求体字段 | - |
| encoding_format | string | 请求体字段 | float / base64 |
| dimensions | integer | 请求体字段 | - |

#### 响应格式

- 透传上游兼容协议响应，不再套用后台 `ApiResponse` 外层。

### POST /v1beta/models/{model}:generateContent

- 摘要：Gemini 图片(Nano Banana)
- 请求方法：`POST`
- 路径：`/v1beta/models/{model}:generateContent`

#### 路径 / Query 参数

| 字段 | 类型 | 含义 | 取值范围 |
|---|---|---|---|
| model | path / string / 必填 | 模型名称 | - |

#### 请求体

- Content-Type：`application/json`
- 请求体结构：`object`

| 字段 | 类型 | 含义 | 取值范围 |
|---|---|---|---|
| contents | array<object> | 请求体字段 | - |
| generationConfig | object | 请求体字段 | - |

#### 响应格式

- 透传上游兼容协议响应，不再套用后台 `ApiResponse` 外层。
